            ...
            match result {
                Ok(()) => {
                    ...
                    if context.client_request.is_executable() {
                        ...
                        // in. Writes to `broadcast_null` tables park with
                        // them — their NULL-key writes broadcast like omni
                        // writes, and their keyed writes are published WAL
                        // that would keep the drain from converging. Writes
                        // park for a MOVE KEYS cutover when they name a
                        // paused sharding key, or name no key at all on a
                        // sharded write (a broadcast can touch moving rows).
                        // In-transaction writes are exempt: they already
                        ...
                        let waiter = if !context.in_transaction() {
                            ...
                            if route.is_write() {
                                if route.is_omnisharded() || route.is_broadcast_null_table() {
