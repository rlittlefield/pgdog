...
pub(super) struct Preflight {
    ...
    /// Hybrid sharded tables: their NULL-key rows
...
/// The cluster's hybrid tables. A hybrid table without a
    ...
    source
        ...
        .map(|table| {
            table
                ...
                .ok_or_else(|| Error::HybridUnnamedTable(table.column.clone()))
        })
        .collect()
}
...
mod test {
    ...
    fn test_hybrid_tables_from_cluster() {
        ...
        use pgdog_config::{ConfigAndUsers, TableKind};
        ...
        assert!(matches!(
            ...
            Err(Error::HybridUnnamedTable(_))
        ));
    }
}
