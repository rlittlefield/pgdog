...
//! rows of hybrid tables — from shard 0, streams WAL until
//! the new shard has caught up, then — on operator `CUTOVER` or
//! automatically — pauses omnisharded and hybrid-table writes
...
//!
//! The destination is the next declared shard: described in its final
//! shape in the config, excluded from the serving topology until the
//! cutover activates it and stamps the cluster's `pgdog.config`
//! markers with the new count.
//!
...
/// Add a new shard to a database: provision the next shard declared
/// in the config, catch it up over logical replication, and activate
/// it in the topology.
...
pub(crate) struct AddShardTask {
    ...
    pub database: String,
    /// The shard being added: names one of the database's pending
    /// entries.
    pub shard: usize,
    /// Operator-supplied publication; when absent, one is created for
    /// the omnisharded and hybrid tables and dropped when
...
impl Task for AddShardTask {
    ...
    async fn run(self, ctx: TaskContext<Self>) -> Result<(), MigrationError> {
        ...
        // With no omnisharded or hybrid tables there
