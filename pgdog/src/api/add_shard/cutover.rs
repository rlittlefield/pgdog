...
async fn drain(waiter: &ReplicationWaiter) -> bool {
    ...
    // In-flight omni and hybrid-table writes finish and replicate;
...
pub(super) async fn finalize(task: &AddShardTask, coordination: Option<&Coordinator>) {
    ...
    info!(
        "[add shard] the config needs no edits: every shard's pgdog.config now \
         records {} shards, which restarts and RELOADs trust",
        task.shard + 1
    );
