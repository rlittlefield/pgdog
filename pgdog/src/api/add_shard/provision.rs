...
/// (`kind = "hybrid"`) tables: operator-supplied ones are validated and
...
pub(super) async fn copy_data(
    ...
    // The pre-data schema sync reloaded the registry, shutting down
    // the pools the source cluster handle points at: re-resolve it
    // before the copy checks out connections.
