...
use crate::backend::fleet::coordination::KeyMovePayload;
use crate::backend::fleet::{Coordinator, Discovery};
use crate::backend::key_move;
    ...
    for (number, cluster_shard) in cluster.shards().iter().enumerate() {
        ...
        for query in &move_queries {
            for key in keys {
                ...
                server
                    .fetch_all::<crate::net::messages::DataRow>(
                        crate::backend::server::ServerRequest::parameterized(query, &params),
                    )
                    .await
                    .map_err(|err| {
                        Error::Lookup(format!(
