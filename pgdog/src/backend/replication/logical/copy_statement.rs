...
pub(crate) struct CopyStatement {
    ...
    /// one set of sharding keys; hybrid tables copy their NULL-key
    /// rows). Applies to the source side only: the destination COPY
    /// receives pre-filtered rows. `COPY (SELECT ...)` can't bind
    /// parameters, so the predicate arrives with its values already
    /// quoted.
...
impl CopyStatement {
    ...
    /// Copy out only rows where the column is NULL (hybrid tables).
