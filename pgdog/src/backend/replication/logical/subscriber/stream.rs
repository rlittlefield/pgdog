...
struct Statements {
    ...
    /// Position of the hybrid table's sharding key in the tuple:
...
impl StreamSubscriber {
    ...
    /// Is the tuple part of a hybrid (`kind = "hybrid"`) table's
    ...
    /// Is the table hybrid-filtered (`kind = "hybrid"`)?
    ...
    async fn insert(&mut self, insert: XLogInsert) -> Result<(), Error> {
        ...
        // Hybrid tables replicate only NULL-key rows.
    ...
    async fn update(&mut self, update: XLogUpdate) -> Result<(), Error> {
        ...
        match update.identity {
            ...
            UpdateIdentity::Old(ref old) => {
                ...
                // Hybrid tables: membership follows the
            ...
            UpdateIdentity::Nothing => {
                ...
                self.check_toasted_identity(&update)?;
                // Hybrid tables: membership follows the new key.
    ...
    async fn delete(&mut self, delete: XLogDelete) -> Result<(), Error> {
        ...
        // Hybrid tables apply every DELETE: keyed rows
                ...
                let null_column_idx = match &table.null_filter_column {
                    Some(column) => Some(
                        table
                            ...
                            .ok_or_else(|| Error::HybridColumnMissing {
                                table: table.table.to_string(),
                                column: column.clone(),
                            })?,
                    ),
