...
pub(crate) struct Publisher {
    ...
    /// (`kind = "hybrid"`); empty outside ADD SHARD.
...
impl Publisher {
    ...
    /// Restrict hybrid tables to their NULL-key rows: the
        ...
        for table in tables {
            ...
            if let Some(hybrid) = hybrid {
                if !table.columns.iter().any(|col| col.name == hybrid.column) {
                    return Err(Error::HybridColumnMissing {
...
mod test {
    ...
    fn annotate_hybrid_tables_sets_filter_and_validates_column() {
        let publisher =
            Publisher::new("test", "slot".into()).with_hybrid_tables(vec![HybridNullTable {
                schema: None,
