...
"#;
        ...
        // The pending state is derived at runtime, never parsed.
        assert!(config.databases.iter().all(|d| !d.provisioning));
        config
            .databases
            .iter_mut()
            .find(|d| d.shard == 2)
        ...
        // The serving topology has 2 shards; the pending entry is
    ...
    #[test]
    fn test_provisioning_not_a_config_key() {
        // The old flag fails loudly instead of silently serving a
        // shard that was meant to stay pending.
