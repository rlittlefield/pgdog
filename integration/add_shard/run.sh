#!/bin/bash
# Integration test: ADD SHARD — provision the declared-but-pending
# shard while omni writes keep flowing, then cut over.
#
# Requires:
...
echo "=== Test E: a declared shard is pending until the cluster confirms it ==="
reset_all
# The config plainly declares shard 2, but shard 0's pgdog.config
# marker says the cluster has 2 shards: a tenant routed to shard 2
# must be refused while it's pending.
app "INSERT INTO orgs (id, shard_id) VALUES ('org_conv', 2)" >/dev/null
if app "INSERT INTO data (org_id, value) VALUES ('org_conv', 'x')" >/dev/null 2>&1; then
    echo "ASSERTION FAILED: a pending shard served a write"
    exit 1
fi
...
# Restart pgdog with the same, unchanged config: the cutover stamped
# pgdog.config on every shard with the new count, so the manifest that
# gated shard 2 before now serves it.
stop_pgdog
start_pgdog
...
assert_eq "${CONVERGED}" "yes" "restarted pgdog serves 3 shards"
...
reset_all
# Same topology, but no omnisharded or hybrid tables declared:
