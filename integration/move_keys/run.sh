...
# Wait until N instances are live in the registry on shard 0.
wait_for_fleet() {
    local expected=$1
...
echo "=== Test D: a second instance invalidates its cache on the flip ==="
reset_all
start_pgdog2
wait_for_fleet 2

# Warm the second instance's lookup cache with the old placement.
WARM=$(app2 "SELECT count(*) FROM data WHERE org_id = 'org_a'")
assert_eq "${WARM}" "2" "second instance reads through the old placement"

TASK_ID=$(admin "MOVE KEYS pgdog 1 org_a AUTO")
wait_for_finished "${TASK_ID}" 120

# The second instance routes to the target without a restart: its
# follower invalidated the cached translation.
app2 "INSERT INTO data (org_id, value) VALUES ('org_a', 'from2')" >/dev/null
ROUTED=""
for _ in $(seq 1 30); do
    ON_TARGET=$(direct pgdog2 "SELECT count(*) FROM data WHERE org_id = 'org_a' AND value = 'from2'")
    if [ "${ON_TARGET}" = "1" ]; then
        ROUTED="yes"
        break
    fi
...
assert_eq "${ROUTED}" "yes" "second instance routed to the target without a restart"
stop_pgdog2

echo "=== Test E: replica identity not covering the key is refused ==="
reset_all
...
echo "=== Test F: foreign keys between moving tables delete in order ==="
reset_all
for db in pgdog1 pgdog2; do
...
echo "=== Test G: non-moving keys never block during the cutover ==="
reset_all
...
echo "=== Test H: an instance started after the move routes fresh ==="
reset_all
TASK_ID=$(admin "MOVE KEYS pgdog 1 org_a AUTO")
wait_for_finished "${TASK_ID}" 120
...
echo "=== Test I: a drain that can't converge re-parks and releases ==="
reset_all
...
echo "=== Test J: hybrid table moves keyed rows, global rows stay put ==="
reset_all
